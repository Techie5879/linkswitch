...
Native macOS app that registers as the `http` / `https` URL handler, routes opens by **source app bundle ID** (first-class case: Slack to Helium with a Chromium-style work profile), and sends everything else to a **user-chosen default browser** stored in config. Minimal preferences UI for the default browser and per-source rules; config persisted explicitly (do not infer the default browser from Launch Services once this app is the handler). Firefox-family default browsers may target real `profiles.ini` profiles, and Zen default-browser routing may target container identities through the explicit extension-based `ext+container:` flow. No domain/URL pattern routing, no chooser UI, no Safari profile support, no generic extension platform support beyond that explicit Zen container handoff, and no interception outside the normal URL-handler path.
