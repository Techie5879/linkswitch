...
Native macOS app that registers as the `http` / `https` URL handler, routes opens by **domain** or **source app bundle ID** (first-class case: Slack to Helium with a Chromium-style work profile), and sends everything else to a **user-chosen default browser** stored in config. Domain rules match a hostname and its subdomains, take precedence over source-app rules, and prefer the most-specific match. Minimal preferences UI for the default browser and both rule types; config persisted explicitly (do not infer the default browser from Launch Services once this app is the handler). Firefox-family default browsers may target real `profiles.ini` profiles, and Zen default-browser routing may target container identities through the explicit extension-based `ext+container:` flow. No URL path/query patterns, no chooser UI, no Safari profile support, no generic extension platform support beyond that explicit Zen container handoff, and no interception outside the normal URL-handler path.
...
- After user-visible UI changes, always capture a fresh screenshot of the relevant app state as part of verification and inspect it for layout, alignment, spacing, clipping, and other visual regressions instead of relying only on code inspection or XCTest assertions.
